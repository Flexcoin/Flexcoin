...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        LogPrint("infinitemoneycoin", "CreateNewBlock(): total size %u\n", nBlockSize);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("infinitemoneycoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("infinitemoneycoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("infinitemoneycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("infinitemoneycoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrint("infinitemoneycoin", "Running infinitemoneycoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
