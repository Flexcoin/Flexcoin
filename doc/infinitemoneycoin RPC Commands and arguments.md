# infinitemoneycoin RPC Commands and arguments
...
The configuration file is not automatically created; you can create it using your favorite plain-text editor. By default, infinitemoneycoin-qt (or infinitemoneycoind) will look for a file named infinitemoneycoin.conf in the infinitemoneycoin data directory, but both the data directory and the configuration file path may be changed using the -datadir and -conf command-line arguments.

| Platform	   |Path to data folder           | Typical path to configuration file                                                 |
|------------|--------------------------------|------------------------------------------------------------------------------------|
| Linux	     | ~/	                            | /home/username/.infinitemoneycoin/infinitemoneycoin.conf                                               |
| macOS      | ~/Library/Application Support/	| /Users/username/Library/Application Support/Eletra/infinitemoneycoin.conf                    |
| Windows	   | %APPDATA%	                    | (Vista-10) C:\Users\username\AppData\Roaming\infinitemoneycoin\infinitemoneycoin.conf                  |
|            |                                | (2000-XP) C:\Documents and Settings\username\Application Data\infinitemoneycoin\infinitemoneycoin.conf |

Note: if running infinitemoneycoin in testnet mode, the sub-folder testnet will be appended to the data directory automatically. Command line arguments

## These commands are accurate as of infinitemoneycoin Core version 2.0.0:

•	infinitemoneycoind

•	infinitemoneycoin-cli

•	infinitemoneycoin-tx

•	infinitemoneycoin-qt

## infinitemoneycoind Options

Start infinitemoneycoin Core Daemon with option:
<table>
<tr> <td>infinitemoneycoind</td><td>[options]</td></tr>
</table>
...
<tr> <td>--conf=<file></td><td>Specify configuration file (default: infinitemoneycoin.conf)</td></tr>
...
<tr> <td>--pid=<file></td><td>Specify pid file (default: infinitemoneycoind.pid)</td></tr>
...
<tr> <td>--reindexmoneysupply</td><td>Reindex the infinitemoneycoin and zECA money supply statistics on startup</td></tr>
...
<tr> <td>--paytxfee=<amt></td><td>Fee (in infinitemoneycoin/kB) to add to transactions you send (default: 0.00)
...
<tr> <td>--minrelaytxfee=<amt></td><td>Fees (in infinitemoneycoin/Kb) smaller than this are considered zero fee for relaying (default: 0.0001)
...
<tr> <td>-litemode=<n></td><td>Disable all infinitemoneycoin specific functionality (Masternodes, Zerocoin, wiftX, Budgeting) (0-1, default: 0)
</table>
...
<tr> <td>--ecastake=<n></td><td>Enable or disable staking functionality for infinitemoneycoin inputs (0-1, default: 1)
...
infinitemoneycoin QT GUI, use same command line options as infinitemoneycoind with additional options for UI as described below.

Start infinitemoneycoin QT GUI:

<table>
<tr> <td>infinitemoneycoin-qt</td><td>[command-line options]</td></tr>
</table>

### Debugging/Testing options

<table>
<tr> <td>--debug=<category></td><td>Output debugging information (default: 0, supplying <category> is optional). If <category> is not supplied or if <category> = 1, output all debugging information.<category> can be: addrman, alert, bench, coindb, db, http, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, infinitemoneycoin (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork), qt.
</table>
...
This documentation lists all available RPC commands as of infinitemoneycoin version 2.0.0, and limited documentation on what each command does. For full documentation of arguments, results and examples, type help ( “command” ) to view full details at the console. You can enter commands either from Tools > Debug console in the QT wallet, or using infinitemoneycoin-cli
...
#### debug ( 0 | 1 | addrman | alert | bench | coindb | db | lock | rand | rpc | selectcoins | mempool | mempoolrej | net | proxy | prune | http | libevent | infinitemoneycoin )
...
Stop infinitemoneycoin Core server.
...
#### preparebudget "proposal-name" "url" payment-count block-start "infinitemoneycoin-address" monthy-payment
...
#### submitbudget "proposal-name" "url" payment-count block-start "infinitemoneycoin-address" monthy-payment "fee-tx"
...
PoW Only: Return if the server is set to generate coins or not. The default is false. It is set with the command line argument -gen (or infinitemoneycoin.conf setting gen) It can also be set with the setgenerate call.
...
#### validateaddress “infinitemoneycoinaddress”

Return information about the given infinitemoneycoin address.

#### verifymessage “infinitemoneycoinaddress” “signature” “message”
...
#### bip38decrypt "infinitemoneycoinaddress" "passphrase"

Decrypts and then imports password protected private key.

#### bip38encrypt "infinitemoneycoinaddress" "passphrase"

Encrypts a private key corresponding to 'infinitemoneycoinaddress'.

#### dumpprivkey “infinitemoneycoinaddress”

Reveals the private key corresponding to ‘infinitemoneycoinaddress’. Then the importprivkey can be used with this output
...
DEPRECATED. Returns the current infinitemoneycoin address for receiving payments to this account.
...
Returns a new infinitemoneycoin address for receiving payments. If ‘account’ is specified (DEPRECATED), it is added to the address book so payments received with the address will be credited to ‘account’.

#### getrawchangeaddress

Returns a new infinitemoneycoin address, for receiving change. This is for use with raw transactions, NOT normal use.

#### getreceivedbyaccount “account” ( minconf addlockconf )

DEPRECATED. Returns the total amount received by addresses with <account> in transactions with specified minimum number of confirmations.

#### getreceivedbyaddress “infinitemoneycoinaddress” ( minconf addlockconf )

Returns the total amount received by the given infinitemoneycoinaddress in transactions with specified minimum number of confirmations.
...
#### importprivkey “infinitemoneycoinprivkey” ( “label” rescan )
...
#### sendfrom “fromaccount” “toinfinitemoneycoinaddress” amount ( minconf addlockconf “comment” “comment-to” )

DEPRECATED (use sendtoaddress). Sent an amount from an account to a infinitemoneycoin address.
...
#### sendtoaddress “infinitemoneycoinaddress” amount ( “comment” “comment-to” subtractfeefromamount use_is use_ps )
...
Stores the wallet decryption key in memory for ‘timeout’ seconds. This is needed prior to performing transactions related to private keys such as sending infinitemoneycoins
...
Usage:
  infinitemoneycoin-tx [options] <hex-tx> [commands]  Update hex-encoded infinitemoneycoin transaction
  infinitemoneycoin-tx [options] -create [commands]   Create hex-encoded infinitemoneycoin transaction
